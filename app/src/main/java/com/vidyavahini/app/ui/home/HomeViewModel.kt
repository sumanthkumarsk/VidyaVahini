...
class HomeViewModel @Inject constructor(
     ...
     */
    fun startDemoSimulation(intervalMs: Long = 8000L) {
        ...
        demoJob = viewModelScope.launch {
            for ((stopId, stop) in sortedStops) {
                demoStopName.postValue("🚌 Demo: Bus now at ${stop.name}")
                repo.pingBus(activeRouteId, stopId)
                delay(intervalMs)
            }
            demoStopName.postValue("🏁 Demo: Bus reached final destination!")
